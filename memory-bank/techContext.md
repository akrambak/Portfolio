...
*   **Development setup:** `npm install`, then `npm run dev` (port 3210). `npm run build` for production; `npm run start` serves on **port 3100**. `npm run deploy` builds as the `bak-dev` user and restarts PM2 — the only supported way to ship. The PM2 process is defined by `ecosystem.config.js` (apply with `npm run pm2:setup`). `npm run lint` runs `eslint-config-next`.
